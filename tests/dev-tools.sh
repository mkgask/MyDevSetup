...
run_helper_with_inputs() {
	...
	printf '%s' "$inputs" | script --quiet --flush --command "env DEV_TOOLS_AGENTS_PATH=\"$MOCK_AGENTS\" HOME=\"$MOCK_HOME\" MOCK_ROOT=\"$MOCK_ROOT\" MOCK_BIN=\"$MOCK_BIN\" MOCK_LOG=\"$MOCK_LOG\" MOCK_FAIL_TOOL=\"$MOCK_FAIL_TOOL\" PATH=\"$PATH\" bash \"$HELPER_PATH\"" "$output_file" > "$output_file.stdout" 2>&1
...
test_interactive_install_and_failure_continuation() {
	...
	output="$(run_helper_with_inputs $'system\nsystem\nsystem\nofficial\nofficial\n')"
...
run_helper_mode() {
	...
	env DEV_TOOLS_AGENTS_PATH="$MOCK_AGENTS" HOME="$MOCK_HOME" MOCK_ROOT="$MOCK_ROOT" MOCK_BIN="$MOCK_BIN" MOCK_LOG="$MOCK_LOG" MOCK_FAIL_TOOL="$MOCK_FAIL_TOOL" PATH="$PATH" bash "$HELPER_PATH" "$mode" > "$output_file" 2>&1
	cat "$output_file"
}
